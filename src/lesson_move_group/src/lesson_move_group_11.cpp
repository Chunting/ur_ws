...
struct Ur5Interface{
  ...
  int ik_newmap3(int side, std::vector<double> current_joint_values,  tfPercNeuronInterface& tfPNIntf, std::map<std::string, double>& joints){
    ...
    for(int j=0;j<2;j++){
      ...
      tf::Transform transElbowWist = transElbowArm.operator*(transform);
  ...
  int ik_newmap4(int side, std::vector<double> current_joint_values,  tfPercNeuronInterface& tfPNIntf, std::map<std::string, double>& joints){
    tf::Vector3 ArmPos;
    double* Joints1=new double[2];
    double* Joints2=new double[2];
    tf::Quaternion Quat;
    ...
    if(side==1){
      ...
      tf::Transform TransBase;
      Quat.setRPY(0.0,PI/2,0.0);
      TransBase.setRotation(Quat);
      TransBase.setOrigin(tf::Vector3(0.0,0.0,0.0));
      ...
      tf::Transform transBaseArm;
      transBaseArm=TransBase.operator*(transform);
      Quat=transBaseArm.getRotation();
      tf::Matrix3x3(Quat).getRPY(roll,pitch,yaw);
      joints["left_shoulder_pan_joint"] = PI/2-yaw;
      joints["left_shoulder_lift_joint"] = -PI+pitch;
      ...
      Quat=transform.getRotation();
      tf::Matrix3x3(Quat).getRPY(roll,pitch,yaw);
      joints["left_wrist_1_joint"] =  pitch;
      joints["left_wrist_2_joint"] =  PI/2-yaw;
      joints["left_wrist_3_joint"] = 0;
  ...
  int findClosestIK(const double* T, double* q_sol, double q6_des, std::vector<double> current_joint_values){
    ...
    for (int i=0; i<a;i++){
      d[i]=d[i]+(q_sols[i*6]+PI/2-current_joint_values.at(0))*(q_sols[i*6]+PI/2-current_joint_values.at(0));
      for (int j=1; j<6;j++){
...
int main(int argc, char **argv)
{
  ...
  double framesPerSecPercNeuron = 5;
 ...
 //Comput the transform from neck to RightBaseLink;
  ...
  while (node.ok()) {
    ...
    if (control_mode==2){
      ...
      joints1["left_shoulder_pan_joint"] = q_sol[0]+PI/2;
    ...
    if (control_mode==3){
       //left
        ...
        // printf("\n getCurrentJointValues" );
        // printf("\n %1.6f,%1.6f\n",current_joint_values.at(0),current_joint_values.at(1) );
        ur.ik_newmap4(side, current_joint_values,tfPNIntfRef,left_ur_joints);
        // joints1["left_shoulder_pan_joint"] = joints1["left_shoulder_pan_joint"]+PI/2;
        group1.setJointValueTarget(joints1);
        if (!group1.plan(plan1))
        {
