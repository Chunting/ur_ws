...
struct tfPercNeuronInterface {
  ...
  int ik_newmap(int side, std::vector<double> current_joint_values,  tfPercNeuronInterface& tfPNIntf, std::map<std::string, double>& joints){
    ...
    if(side==1){
      ...
      // transArmForearm.setBasis(transform.getBasis());
      // transArmForearm.setOrigin(origin_arm_forearm);
...
struct Ur5Interface{
  ...
  int ik_newmap2(int side, std::vector<double> current_joint_values,  tfPercNeuronInterface& tfPNIntf, std::map<std::string, double>& joints){
    ...
    for(int j=0;j<2;j++){
      ...
      joints["left_wrist_2_joint"] =  PI/2+pitch-0.2;
      joints["left_wrist_3_joint"] = -0.8+roll;
  ...
  int ik_newmap3(int side, std::vector<double> current_joint_values,  tfPercNeuronInterface& tfPNIntf, std::map<std::string, double>& joints){
    tf::Vector3 ArmPos;
    double* Joints1=new double[2];
    double* Joints2=new double[2];
    tf::Quaternion Quat;
    ...
    for(int j=0;j<2;j++){
      ...
      tf0.setBasis(rot_shoulder_arm);
      tf0.setOrigin(tf::Vector3(0.0,0.0,0.0));
      //origin_shoulder_arm = tf0*(origin_shoulder_arm);
      origin_arm_forearm = tf0*(origin_arm_forearm);
      double elbow_angle = -1*(origin_shoulder_arm.angle(origin_arm_forearm));
      //printf("\n the elbow angle is %f\n", elbow_angle);
      joints["left_elbow_joint"] = elbow_angle;
      ...
      tf::Transform transElbowWist = transElbowArm.operator*(transArmForearm);
      ...
      // transElbowWist = transElbowWist.operator*(transform);

      // tfBroadcaster.sendTransform(tf::StampedTransform(transElbowWist, ros::Time::now(),
      //                       "/LeftShoulderElbow","/LeftElbowWrist"));
      ...
      matrix2XZYEuler(T, euler);
        // d=euler[2]-current_joint_values.at(5);
        // if(d>PI){
        //   euler[2]=Joints2[2]-2*PI;
        // }
        // if(d<-1*PI){
        //   euler[2]=Joints2[2]+2*PI;
        // }
  ...
  int ik_newmap4(int side, std::vector<double> current_joint_values,  tfPercNeuronInterface& tfPNIntf, std::map<std::string, double>& joints, double* human_data){
    tf::Vector3 ArmPos;
    double* Joints1=new double[2];
    double* Joints2=new double[2];
    tf::Quaternion Quat;
    ...
    if(side==1){
      ...
      //
      tf::Transform transLeftArmLeftForeArm;
      transLeftArmLeftForeArm.setBasis(transform.getBasis());
      transLeftArmLeftForeArm.setOrigin(transform.getOrigin());
      //
      forearm=transform.getOrigin();
      RotArmForearm.setBasis(transform.getBasis());
      RotArmForearm.setOrigin(tf::Vector3(0,0,0));//
      ...
      tf::Transform transElbowWist = transElbowArm.operator*(transLeftArmLeftForeArm);
      ...
      // hand=transform.getOrigin();
      // double hand_length=sqrt(hand.dot(hand));
      // printf("the hand length is %f", hand_length);
      hand.setX(0.04);
      hand.setY(0.0);
      hand.setZ(-0.04);
...
int main(int argc, char **argv)
{
  printf("\n Please control mode. \n");
  cin>>control_mode;
  if (control_mode!=1 && control_mode!=2  && control_mode!=3){
    return 0;
  }else{
 ...
 //Comput the transform from neck to RightBaseLink;
  ...
  while (node.ok()) {
    ...
    if (control_mode==2){
      ...
      tfPNIntf.tfBroadcaster.sendTransform(tf::StampedTransform(tfPNIntf.transNeckLeftBaseLink, ros::Time::now(), "/Neck","/LeftBaseLink"));
      ...
      // tf::Transform transLeftHand;
      // q.setRPY(PI/2, 0.0, PI/2);
      // transLeftHand.setRotation(q);
      // transLeftHand.setOrigin(tf::Vector3(0, 0, 0));
      // tfPNIntf.transLeftGoalPosition=tfPNIntf.transLeftGoalPosition.operator*(transLeftHand);
      int i = 0;
      // int scale=-1;
    ...
    if (control_mode==3){
       //left
        ...
        // tf::Transform transLeftUrWorldLeftHandWorld=transLeftHandWorldLeftUrWorld.inverse();
        // arm=transLeftUrWorldLeftHandWorld*(RotNeckShoulder*(arm));
        // forearm=transLeftUrWorldLeftHandWorld*(RotNeckShoulder*(RotShoulderArm*(forearm)));
        // wrist_rot=transLeftUrWorldLeftHandWorld.operator*(RotNeckShoulder.operator*(RotShoulderArm.operator*(RotArmForearm))).getBasis();
        // wrist_rot.getRPY(roll,pitch,yaw);
        ...
        // count_human++; human << count_human<< " ";
        // human << arm.getX()<<" "<<arm.getY()<<" "<<arm.getZ()<<" "; 
        // human << forearm.getX()<<" "<<forearm.getY()<<" "<<forearm.getZ()<<" ";
        // forearmPos=arm.operator+=(forearm);
        // human << forearmPos.getX()<<" "<<forearmPos.getY()<<" "<<forearmPos.getZ()<<" ";
        // human << roll <<" "<< pitch<<" "<< yaw <<" ";
        // human << "\n" << endl;
        ...
        current_joint_values= group1.getCurrentJointValues();
        double* human_data = new double[12];
        // ur.ik_newmap4(side, current_joint_values,tfPNIntfRef,left_ur_joints, human_data);
        ...
        clock_t start = clock();
        // tfPNIntf.ik_newmap(side, current_joint_values,tfPNIntfRef,left_ur_joints);
